/*
 * This is a bookstore inspired by GNDecoration Copyright (C) 2018 Gleidson Neves da Silveira
 */
package vulembere;

import buttons.Close;
import buttons.GNFullscreen;
import buttons.Maximize;
import buttons.Minimize;
 ...
 *
 * @author Willson vulembere Antoine | willsonantoine@gmail.com Created on
 * 06/03/2020. Copie originale : Gleidson Neves da Silveira 
 */
@SuppressWarnings("unused")
public class vulembereGUI {
    ...
    private final BooleanProperty resizableProperty = new SimpleBooleanProperty(vulembereGUI.this, "resizableProperty", true);
    private final StringProperty titleProperty = new SimpleStringProperty(vulembereGUI.this, "textProperty", "title");
    private final BooleanProperty maximizedProperty = new SimpleBooleanProperty(vulembereGUI.this, "maximizedProperty", false);

    private DoubleProperty barHeight = new SimpleDoubleProperty(vulembereGUI.this, "barSize", 30);
    private DoubleProperty buttonHeight = new SimpleDoubleProperty(vulembereGUI.this, "buttonHeight", 30);
    private DoubleProperty buttonWidth = new SimpleDoubleProperty(vulembereGUI.this, "buttonWidth", 30);
    ...
    public enum Theme {
        DEFAULT, DARK, DANGER, INFO, PRIMARY, SECONDARY, WARNING, SUCCESS, CUSTOM
    }

    private final ChangeListener<Object> restoreFullScreen = (observable, oldValue, newValue) -> {
        if (newValue != null) {
            configCursor(true);
            AnchorPane.setTopAnchor(vulembereGUI.this.areaContent, barHeight.get());
            btn_fullScreen.updateState(true);
            if (!vulembereGUI.this.bar.isVisible()) {
                vulembereGUI.this.bar.setVisible(true);
                viewBorders(true);
            }
...
//        menus.getChildren().add(node);
    ...
    /**
     * Centralize title. Center the title using the half-width of the stage and
     * subtract the width of the right and left controls
     */
     ...
     * limit must be set. perfect for sliding applications.
     *
    ...
    /**
     * This method position actions maximize, minimize and close. The bar is set
     * to a minimum size to contain only your children and is position in left.
     */
...
//        bar.toFront();
    ...
    /**
     * Restaura o tanho da janela | Restores size of the window.. Se a decoração
     * inicial maximizavel ao restaurar o tamanho é decrecido em 10, no outro
     * caso o tamanho é restaurado para antes de maximizar | f the initial
...
//
    ...
    private void viewBorders(boolean view) {
        if (view && !isMaximized() && !stage.isFullScreen()) {
            this.background.setStyle("-fx-border-width : 1");
        } else {
            this.background.setStyle("-fx-border-width : 0");
        }
    }
    ...
    private void configFullEffect() {
        ...
        stage.fullScreenProperty().addListener((ObservableValue<? extends Object> observable, Object oldValue, Object newValue) -> {
            if (newValue != null) {
                configCursor(true);
                viewBar(true);
                if (vulembereGUI.this.controls.getChildren().size() < 4) {
                    vulembereGUI.this.controls.getChildren().addAll(btn_minimize, btn_maximize);
                    btn_maximize.toFront();
                    btn_close.toFront();
    ...
    public void initTheme(Theme theme) {
        switch (theme) {
            case DEFAULT:
                ...
                this.background.getStylesheets().add(getClass().getResource("/theme/default.css").toExternalForm());
                break;
            case DARK:
                this.background.getStylesheets().clear();
                this.background.getStylesheets().add(getClass().getResource("/theme/darkula.css").toExternalForm());
                break;
            case DANGER:
                this.background.getStylesheets().clear();
                // add theme
                this.background.getStylesheets().add(getClass().getResource("/theme/danger.css").toExternalForm());
                break;
            case INFO:
                this.background.getStylesheets().clear();
                // add theme
                this.background.getStylesheets().add(getClass().getResource("/theme/info.css").toExternalForm());
                break;
            case PRIMARY:
                this.background.getStylesheets().clear();
                // add css
                this.background.getStylesheets().add(getClass().getResource("/theme/primary.css").toExternalForm());
                break;
            case SECONDARY:
                this.background.getStylesheets().clear();
                // add css
                this.background.getStylesheets().add(getClass().getResource("/theme/secondary.css").toExternalForm());
                break;
            case SUCCESS:
                this.background.getStylesheets().clear();
                // add css
                this.background.getStylesheets().add(getClass().getResource("/theme/success.css").toExternalForm());
                break;
            case WARNING:
                this.background.getStylesheets().clear();
                // add css
                this.background.getStylesheets().add(getClass().getResource("/theme/warning.css").toExternalForm());
                break;
            case CUSTOM:
                this.background.getStylesheets().clear();
                // add css
                this.background.getStylesheets().add(getClass().getResource("/theme/custom.css").toExternalForm());
                break;
        }
    }
